...
export function sanitizeInput(value, maxLength = 255, { truncate = true } = {}) {
...
export function validateZip(value) {
  if (!getDigits(value)) return "ZIP code is required";
  if (!isValidZip(value, { required: true })) return ZIP_VALIDATION_MESSAGE;
  return "";
}
...
export function validateFacilityForm(form = {}) {
  ...
  } else if (!isValidZip(sanitized.facilityZip)) {
    errors.facilityZip = errors.facilityZip || ZIP_VALIDATION_MESSAGE;
  }
