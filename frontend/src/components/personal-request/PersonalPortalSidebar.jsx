...
import { logoutPersonal } from "@/lib/personal-request/personalPortalAuthApi";

const navItems = [
  { label: "Dashboard", href: "/personalrequest/dashboard", icon: <DashboardIcon /> },
  { label: "New Request", href: "/personalrequest/new", icon: <OrdersIcon /> },
  { label: "My Requests", href: "/personalrequest/requests", icon: <TrackIcon /> },
  { label: "Check Status", href: "/personalrequest/status", icon: <SearchIcon /> },
  { label: "Profile", href: "/personalrequest/profile", icon: <ProfileIcon /> },
];

export default function PersonalPortalSidebar({ isCollapsed }) {
  ...
  const handleLogout = async () => {
    ...
    try {
      ...
      router.push("/personalrequest/login");
      ...
      <nav className="flex-1 px-[10px] py-[16px]">
        <div className="space-y-[7px]">
          {navItems.map((item) => {
            const isActive =
              pathname === item.href ||
              (item.href !== "/personalrequest/dashboard" &&
